...
class BaseTextSelfAttention(nn.Module):
...
class BaseTextAttentionProduct(nn.Module):
...
class CausalTextAttentionProduct(nn.Module):
...
class LSGAttentionProduct(nn.Module):
    ...
    def __init__(self, config, block_size=None, sparse_block_size=None, sparsity_factor=4, is_causal=False):
        ...
        if is_causal:
            self.attention = CausalTextAttentionProduct(config)
        else:
            self.attention = BaseTextAttentionProduct(config)
...
class LSGBlipTextEmbeddings(BlipTextEmbeddings):
...
class LSGSelfAttention(BaseSelfTextAttention):
    '''
    ...
    def __init__(self, config):
        ...
        if self.is_causal:
            self.causal_attention = CausalTextAttentionProduct(config)
        self.full_attention = BaseTextAttentionProduct(config)
...
class LSGAttention(BlipTextAttention):
    ...
    def __init__(self, config):
        ...
        self.output = BlipSelfOutput(config)
        self.pruned_heads = set()


class LSGBlipLayer(BlipEncoderLayer):
...
class LSGBlipEncoder(BlipEncoder):

    def __init__(self, config):

        super().__init__(config)

        self.layers = nn.ModuleList([LSGBlipLayer(config) for _ in range(config.num_hidden_layers)])
...
class LSGBlipPreTrainedModel(BlipPreTrainedModel):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, (BlipTextEncoder, LSGBlipEncoder)):
            module.gradient_checkpointing = value


class LSGBlipTextModel(LSGBlipPreTrainedModel, BlipTextPreTrainedModel):
    """
    This class overrides :class:`~transformers.BlipTextPreTrainedModel`. Please check the superclass for the appropriate
    documentation alongside usage examples.
    """
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        LSGBlipPreTrainedModel.__init__(self, config)

        self.config = config

        self.embeddings = LSGBlipTextEmbeddings(config)
        self.encoder = LSGBlipEncoder(config)
...
class LSGBlipLMHeadModel(LSGBlipPreTrainedModel, BlipTextLMHeadModel):
    ...
    def __init__(self, config):
        ...
        self.bert = LSGBlipModel(config, add_pooling_layer=False)
